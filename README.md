...
###Secure Connection
The [W3C specifications]( http://www.w3.org/TR/service-workers/#security-considerations ) for SWs recommend that they SHOULD be implemented over HTTPS-only. The reasons provided do not seem specific to SWs, but more so apply to best practices. My experience shows me that SWs will work over a non-secure network connection locally, but not over an internet connection. [Github.io]( https://pages.github.com/ ) pages supports HTTPS and was mentioned in multiple resources, as a good place to host dev projects.
